    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-[#f5f8fd] text-[var(--slot4-navy)]">
        <section className="wire-hero-bg text-white">
          <div className="mx-auto flex max-w-[1660px] flex-col gap-6 px-4 py-14 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-10 lg:py-20 xl:px-12">
            <div>
              <span className="wire-yellow-rule" />
              <p className="mt-6 text-xs font-black uppercase tracking-[0.28em] text-white/70">The newsroom</p>
              <h1 className="mt-3 text-5xl font-light leading-none tracking-[-0.055em] sm:text-7xl lg:text-8xl">
                {category === 'all' ? label : categoryLabel}
              </h1>
            </div>
            <p className="max-w-xl border-l-4 border-[var(--slot4-accent)] pl-5 text-lg font-semibold leading-8 text-white/80">
              Timely press releases, brand announcements, and media-ready stories organized for fast discovery.
            </p>
          </div>
        </section>

        <section className="bg-white shadow-[0_1px_0_rgba(25,23,85,.12)]">
          <div className="mx-auto flex max-w-[1660px] gap-3 overflow-x-auto px-4 py-5 text-sm font-black sm:px-6 lg:px-10 xl:px-12">
            <Link href={basePath} className={`shrink-0 rounded-full px-5 py-2 ${category === 'all' ? 'bg-[var(--slot4-navy)] text-white' : 'bg-[#eef3fb] hover:bg-[var(--slot4-accent)]'}`}>Latest</Link>
            {categories.slice(0, 8).map((item) => (
              <Link key={item.slug} href={pageHref(basePath, item.slug, 1)} className={`shrink-0 rounded-full px-5 py-2 ${category === item.slug ? 'bg-[var(--slot4-navy)] text-white' : 'bg-[#eef3fb] hover:bg-[var(--slot4-accent)]'}`}>
                {item.name}
              </Link>
            ))}
          </div>
        ...
        {lead ? (
          <section className="mx-auto grid max-w-[1660px] gap-7 px-4 py-10 sm:px-6 lg:grid-cols-[1.5fr_.7fr] lg:px-10 xl:px-12">
            <Link href={`${basePath}/${lead.slug}`} className="group wire-card-hover relative min-h-[34rem] overflow-hidden rounded-xl bg-white shadow-sm">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-[var(--slot4-navy)] via-[rgba(25,23,85,.12)] to-transparent" />
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                <span className="rounded-full bg-[var(--slot4-accent)] px-4 py-2 text-[10px] font-black uppercase tracking-[0.2em] text-[var(--slot4-navy)]">{getCategory(lead, label)}</span>
            ...
            <div className="grid gap-5">
              <div className="rounded-xl bg-[var(--slot4-blue)] p-6 text-white">
              ...
              {secondary.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group wire-card-hover grid grid-cols-[7rem_1fr] overflow-hidden rounded-xl bg-white shadow-sm">
                  <img src={getImage(post)} alt="" className="h-full min-h-40 w-full object-cover transition group-hover:scale-105" />
                  <div className="p-5">
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--slot4-blue)]">0{index + 1}</p>
        ...
        <section className="mx-auto max-w-[1660px] px-4 pb-16 sm:px-6 lg:px-10 xl:px-12">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-5">
            <div>
              <span className="wire-yellow-rule" />
              <h2 className="mt-5 text-4xl font-black tracking-[-0.04em] sm:text-5xl">More from the desk</h2>
            </div>
            <form action={basePath} className="flex overflow-hidden rounded-full border-2 border-[var(--slot4-navy)] bg-white">
              ...
              <button className="h-12 bg-[var(--slot4-navy)] px-5 text-xs font-black uppercase tracking-[0.14em] text-white">Filter</button>
            </form>
          </div>
          ...
          {remaining.length ? (
            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group wire-card-hover overflow-hidden rounded-xl bg-white shadow-sm">
                  <div className="aspect-[16/10] overflow-hidden bg-black">
                    <img src={getImage(post)} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
                  </div>
                  <div className="p-5">
                    <div className="flex items-center justify-between gap-4 text-[10px] font-black uppercase tracking-[0.18em] text-[var(--slot4-blue)]">
            ...
            <div className="rounded-2xl border border-dashed border-[var(--slot4-navy)]/30 bg-white p-12 text-center">
              <Search className="mx-auto h-8 w-8" />
              <h2 className="mt-4 text-3xl font-black">No stories found</h2>
              <p className="mt-2 text-sm text-slate-600">Try another category or publish a new newsroom story.</p>
            </div>
          ) : null}

          <div className="mt-10 flex items-center justify-center gap-0">
            {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="rounded-l-full border-2 border-[var(--slot4-navy)] bg-white px-5 py-3 text-xs font-black uppercase">Previous</Link> : null}
            <span className="border-y-2 border-[var(--slot4-navy)] bg-[var(--slot4-blue)] px-5 py-3 text-xs font-black uppercase text-white">Page {page} / {pagination.totalPages || 1}</span>
            {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="rounded-r-full border-2 border-[var(--slot4-navy)] bg-white px-5 py-3 text-xs font-black uppercase">Next</Link> : null}
          </div>
        </section>
      </main>
    </EditableSiteShell>
